...
def load_session(year: int, race: str, session_type: str = 'R'):
    ...
    # ── Cache setup ───────────────────────────────────────────
    # On Streamlit Cloud the working directory is read-only.
    # We use /tmp which is always writable on any Linux server.
    # Locally this also works fine — /tmp is available everywhere.
    ...
    if session.laps is None or len(session.laps) == 0:
        raise ValueError(
            ...
            f"This race may not be available yet."
        )
