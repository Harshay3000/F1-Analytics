...
# ── Cache directory ───────────────────────────────────────────
# Use /tmp so this works on both local machines AND Streamlit Cloud.
# Streamlit Cloud's working directory is read-only, but /tmp is
# always writable on any Linux server including Streamlit Cloud.
# On Windows locally, tempfile.gettempdir() returns C:\Users\...\AppData\Local\Temp
...
def load_session(year: int, race: str, session_type: str = 'R'):
    ...
    # Load with telemetry=False first for resilience on Streamlit Cloud.
    # Telemetry (X/Y/Speed etc) is fetched separately per-driver when
    # needed — loading it globally often times out on restricted networks.
    try:
        ...
        raise RuntimeError(
            f"FastF1 failed to load {year} {race} ({session_type}). "
            f"Underlying error: {load_err}"
        ) from load_err

    # Access laps via _laps directly to avoid the property guard
    # that raises DataNotLoadedError on empty sessions
    try:
        raw_laps = session._laps
    except Exception:
        raw_laps = None

    if raw_laps is None or len(raw_laps) == 0:
        raise ValueError(
            f"No lap data returned for {year} {race} {session_type}. "
            f"This session may not be available yet or the race name "
            f"may be misspelled."
        )
