...
<style>
  ...
  /* Hide Streamlit's auto page nav — we render our own below */
...
with st.sidebar:
    ...
    pages = [
        ...
        ('Driver Comparison', '/driver_comparison'),
        ('Circuit Map',       '/circuit_map'),
        ('Gap and Race Pace', '/gap_and_pace'),
        ('Driver Rating',     '/driver_rating'),
        ('Tyre Degradation',  '/tyre_degradation'),
        ('Pit Strategy',      '/pit_strategy'),
        ('AI Narrator',       '/ai_narrator'),
    ]

    for name, path in pages:
        st.markdown(f"""
        <div style='padding:6px 10px;border-radius:6px;margin-bottom:2px;
                    color:#aaaacc;font-size:13px;cursor:pointer;
...
if not st.session_state.get('session_loaded'):
    ...
    for col, title, desc in [
        ...
        (col3, 'Driver Comparison',
         'Head-to-head telemetry — speed, throttle and brake aligned by distance.'),
    ]:
        with col:
            st.markdown(f"""
    ...
    try:
        ...
        event_title = f"{st.session_state['loaded_year']} {st.session_state['loaded_race']}"
        event_date  = ''
